...
namespace Umbraco.Core.Persistence.Migrations.Upgrades.TargetVersionSevenFifteenSeven
{
    [Migration("7.15.7", 1, Constants.System.UmbracoMigrationName)]
        ...
        public override void Up()
        {
            Execute.Code(database =>
            {
                ...
                foreach (var row in database.Query<dynamic>(sql, new { propertyEditorAlias = "Umbraco.UploadField", nodeObjectType = Constants.ObjectTypes.Media }))
                {
                    ...
                    var mediaPath = (string)(string.IsNullOrEmpty(row.dataNvarchar) ? row.dataNtext : row.dataNvarchar);

                    if (string.IsNullOrWhiteSpace(mediaPath) == false && versionsDone.Add(versionId))
                    {
                ...
                foreach (var row in database.Query<dynamic>(sql, new { propertyEditorAlias = "Umbraco.ImageCropper", nodeObjectType = Constants.ObjectTypes.Media }))
                {
                    ...
                    var value = (string)(string.IsNullOrEmpty(row.dataNvarchar) ? row.dataNtext : row.dataNvarchar);
