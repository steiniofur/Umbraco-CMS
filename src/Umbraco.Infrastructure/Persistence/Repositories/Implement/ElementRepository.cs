...
// todo element update naming/documentation, cleanup unused things
...
public class ElementRepository : ContentRepositoryBase<int, IElement, ElementRepository>, IElementRepository
{
    ...
    private PermissionRepository<IElement>? _permissionRepository;
    ...
    // todo element: make PersmissionRepository work with lowest shared interface between IContent and IElement or IElement if IContent because a superclass of IElement
    // note: is ok to 'new' the repo here as it's a sub-repo really
    // private PermissionRepository<IElement> PermissionRepository => _permissionRepository
    //                                                                ?? (_permissionRepository =
    //                                                                    new PermissionRepository<IElement>(
    //                                                                        _scopeAccessor, _appCaches,
    //                                                                        _loggerFactory
    //                                                                            .CreateLogger<
    //                                                                                PermissionRepository<IElement>>()));

    /// <inheritdoc />
    public ContentScheduleCollection GetContentSchedule(int elementId)
    {
        ...
        List<ContentScheduleDto>? scheduleDtos = Database.Fetch<ContentScheduleDto>(Sql()
            ...
            .Where<ContentScheduleDto>(x => x.NodeId == elementId));
    ...
    protected override string ApplySystemOrdering(ref Sql<ISqlContext> sql, Ordering ordering)
    {
        ...
        // => we should probably have an updater per variant (culture/segment pair) if this audit information is deemed important
        ...
        if (ordering.OrderBy.InvariantEquals("published"))
        {
            ...
            {
                return SqlSyntax.GetFieldName<ElementDto>(x => x.Published);
            }
            ...
            var sqlText = InsertBefore(sql.SQL, "FROM",
                ...
                // when invariant, ie 'variations' does not have the culture flag (value 1), it should be safe to simply use the published flag on umbracoElement,
                // otherwise check if there's a version culture variation for the lang, via ccv.id
                $", (CASE WHEN (ctype.variations & 1) = 0 THEN ({SqlSyntax.GetFieldName<ElementDto>(x => x.Published)}) ELSE (CASE WHEN ccvp.id IS NULL THEN 0 ELSE 1 END) END) AS ordering "); // trailing space is important!
    ...
    private IEnumerable<IElement> MapDtosToElement(List<ElementDto> dtos,
        bool withCache = false,
        bool loadProperties = true,
        bool loadVariants = true)
    {
        var temps = new List<TempContent<Element>>();
        var contentTypes = new Dictionary<int, IContentType?>();
        var templateIds = new List<int>();

        var elements = new Element[dtos.Count];

        for (var i = 0; i < dtos.Count; i++)
        {
            // todo elements investigate: why can't we use the singular private method here?
            ...
            if (withCache)
            {
                ...
                if (cached != null && cached.VersionId == dto.DocumentVersionDto.ContentVersionDto.Id) //todo element figure out of we need a seperate ElementVersionDto or if this is actually a generic ContentVersionDto
                {
                    elements[i] = (Element)cached;
                    continue;
                }
            }
            ...
            Element c = elements[i] = ContentBaseFactory.BuildEntity(dto, contentType);
            ...
            var temp = new TempContent<Element>(dto.NodeId, versionId, publishedVersionId, contentType, c)
            {
        ...
        foreach (TempContent<Element> temp in temps)
        {
            // set properties
            if (loadProperties)
            {
        ...
        if (loadVariants)
        {
            ...
            if (temps.Count > 0)
            {
                ...
                IDictionary<int, List<ContentVariation>> contentVariations = GetContentVariations(temps); //todo elements investigate: what is the difference between content and document variations
    ...
    private IElement MapDtoToElement(ElementDto dto)
    {
        IContentType? contentType = _contentTypeRepository.Get(dto.ContentDto.ContentTypeId);
        Element element = ContentBaseFactory.BuildEntity(dto, contentType);

        try
        {
            element.DisableChangeTracking();
            ...
            element.Properties = properties[dto.DocumentVersionDto.Id];
            ...
            if (contentType?.VariesByCulture() ?? false)
            {
                ...
                SetVariations(element, contentVariations, documentVariations);
            }
        ...
        {
            element.EnableChangeTracking();
        }
    }

    private void SetVariations(Element? element, IDictionary<int, List<ContentVariation>> contentVariations,
        IDictionary<int, List<DocumentVariation>> documentVariations)
    {
        ...
        if (contentVariations.TryGetValue(element.VersionId, out List<ContentVariation>? contentVariation))
        {
            foreach (ContentVariation v in contentVariation)
            {
                element.SetCultureInfo(v.Culture, v.Name, v.Date);
            }
        }

        if (element.PublishedVersionId > 0 &&
            contentVariations.TryGetValue(element.PublishedVersionId, out contentVariation))
        {
            foreach (ContentVariation v in contentVariation)
            {
                element.SetPublishInfo(v.Culture, v.Name, v.Date);
            }
        }

        if (documentVariations.TryGetValue(element.Id, out List<DocumentVariation>? documentVariation))
        {
            element.SetCultureEdited(documentVariation.Where(x => x.Edited).Select(x => x.Culture));
        }
    }
    ...
    private IEnumerable<DocumentCultureVariationDto> GetDocumentVariationDtos(IElement element,
    ...
    // todo move into own file as both document and element repos need this, or build up inheritance and mark protected/internal
    ...
    // todo elements should be called PublishableVariation?
    ...
    protected override IElement? PerformGet(int id)
    {
        Sql<ISqlContext> sql = GetBaseQuery(QueryType.Single)
            .Where<NodeDto>(x => x.NodeId == id)
            .SelectTop(1);

        ElementDto? dto = Database.Fetch<ElementDto>(sql).FirstOrDefault();
        return dto == null
            ? null
            : MapDtoToElement(dto);
    }

    protected override IEnumerable<IElement> PerformGetAll(params int[]? ids)
    {
        ...
        return MapDtosToElement(Database.Fetch<ElementDto>(sql));
    }

    protected override IEnumerable<IElement> PerformGetByQuery(IQuery<IElement> query)
    {
        Sql<ISqlContext> sqlClause = GetBaseQuery(QueryType.Many);

        var translator = new SqlTranslator<IElement>(sqlClause, query);
            ...
            case QueryType.Many:
                ...
                sql = sql.Select<ElementDto>(r =>
                        r.Select(elementDto => elementDto.ContentDto, r1 =>
                            ...
                            .Select(elementDto => elementDto.DocumentVersionDto, r1 =>
    ...
    public override IEnumerable<IElement> GetAllVersions(int nodeId)
    {
        ...
        return MapDtosToElement(Database.Fetch<ElementDto>(sql), true);
    }
    ...
    public override IEnumerable<IElement> GetAllVersionsSlim(int nodeId, int skip, int take)
    {
        Sql<ISqlContext> sql = GetBaseQuery(QueryType.Many, false)
            .Where<NodeDto>(x => x.NodeId == nodeId)
            .OrderByDescending<ContentVersionDto>(x => x.Current)
    ...
    public override void DeleteVersion(int versionId)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get("Umbraco.Core.ElementRepository.GetVersion", tsql =>
    ...
    public override void DeleteVersions(int nodeId, DateTime versionDate)
    {
        ...
        SqlTemplate template = SqlContext.Templates.Get("Umbraco.Core.ElementRepository.GetVersions", tsql =>
            tsql.Select<ContentVersionDto>()
                .From<ContentVersionDto>()
                .InnerJoin<DocumentVersionDto>()
                .On<ContentVersionDto, DocumentVersionDto>((c, d) => c.Id == d.Id)
    ...
    protected override void PersistNewItem(IElement entity)
    {
        entity.AddingEntity();
    ...
    protected override void PersistUpdatedItem(IElement entity)
    {
    ...
    protected override void PersistDeletedItem(IElement entity)
    {
    ...
    // public void AssignEntityPermission(IElement entity, char permission, IEnumerable<int> groupIds) =>
        ...
        IQuery<IElement>? filter, Ordering? ordering)
    {
        ...
        return GetPage<ElementDto>(query, pageIndex, pageSize, out totalRecords,
            x => MapDtosToElement(x),
            filterSql,
            ordering);
    }

    public bool IsPathPublished(IElement? element)
    {
        // fail fast
        if (element?.Path.StartsWith("-1,-20,") ?? false)
        {
            return false;
        }
        ...
        IEnumerable<int>? ids = element?.Path.Split(Constants.CharArrays.Comma).Skip(1)
    ...
    public bool RecycleBinSmells()
    {
        ...
        var cacheKey = CacheKeys.ElementRecycleBinCacheKey;
    ...
    public IElement? Get(Guid id) => _contentByGuidReadRepository.Get(id);

    IEnumerable<IElement> IReadRepository<Guid, IElement>.GetMany(params Guid[]? ids) =>
    ...
    private class ContentByGuidReadRepository : EntityRepositoryBase<Guid, IElement>
    {
        private readonly ElementRepository _outerRepo;

        public ContentByGuidReadRepository(ElementRepository outerRepo, IScopeAccessor scopeAccessor, AppCaches cache,
            ILogger<ContentByGuidReadRepository> logger)
            : base(scopeAccessor, cache, logger) =>
            _outerRepo = outerRepo;

        protected override IElement? PerformGet(Guid id)
        {
            Sql<ISqlContext> sql = _outerRepo.GetBaseQuery(QueryType.Single)
                .Where<NodeDto>(x => x.UniqueId == id);

            ElementDto? dto = Database.Fetch<ElementDto>(sql.SelectTop(1)).FirstOrDefault();
            ...
            IElement content = _outerRepo.MapDtoToElement(dto);

            return content;
        }

        protected override IEnumerable<IElement> PerformGetAll(params Guid[]? ids)
        {
            ...
            return _outerRepo.MapDtosToElement(Database.Fetch<ElementDto>(sql));
        }
    ...
    /// <inheritdoc />
    public IEnumerable<IElement> GetElementsForRelease(DateTime date)
    {
    ...
    /// <inheritdoc />
    public IEnumerable<IElement> GetElementsForExpiration(DateTime date)
    {
    ...
    private void SanitizeNames(IElement element, bool publishing)
    {
        // todo element figure out requirements for local/global elements
    ...
    private void EnsureInvariantNameExists(IElement element)
    {
        if (element.IsLocal())
        {
            return;
        ...
        if (element.ContentType.VariesByCulture())
        {
            ...
            element.Name = defaultCulture != null &&
                           (element.CultureInfos?.TryGetValue(defaultCulture, out ContentCultureInfos cultureName) ??
                            false)
                ? cultureName.Name!
                : element.CultureInfos![0].Name!;
        }
        else
        {
    ...
    private void EnsureInvariantNameIsUnique(IElement element) =>
        element.Name = EnsureUniqueNodeName(element.ParentId, element.Name, element.Id);
    ...
    private void EnsureVariantNamesAreUnique(IElement element, bool publishing)
    {
        if (!EnsureUniqueNaming || !element.ContentType.VariesByCulture() || element.CultureInfos?.Count == 0)
        {
            return;
        }

        // get names per culture, at same level (ie all siblings)
        Sql<ISqlContext> sql = SqlEnsureVariantNamesAreUnique.Sql(true, NodeObjectTypeId, element.ParentId, element.Id);
        ...
        foreach (ContentCultureInfos cultureInfo in element.CultureInfos)
        {
            ...
            if (uniqueName == element.GetCultureName(cultureInfo.Culture))
            {
                continue;
            }

            // update the name, and the publish name if published
            element.SetCultureName(uniqueName, cultureInfo.Culture);
            if (publishing && (element.PublishCultureInfos?.ContainsKey(cultureInfo.Culture) ?? false))
            {
                element.SetPublishInfo(cultureInfo.Culture, uniqueName,
