...
/// </summary>
public class ElementRepository : ContentRepositoryBase<int, IContent, ElementRepository>, IElementRepository
{
    ...
    /// <summary>
    ///     Local elements will have no names at all
    /// </summary>
    protected virtual bool EnsureUniqueNaming { get; } = false;
    ...
    private IEnumerable<IContent> MapDtosToContent(List<ElementDto> dtos,
        bool withCache = false,
        bool loadProperties = true)
        //bool loadVariants = true) //todo variants
    {
        ...
        for (var i = 0; i < dtos.Count; i++)
        {
            ...
            if (withCache)
            {
                ...
                if (cached != null) // && cached.VersionId == dto.DocumentVersionDto.ContentVersionDto.Id) //todo global blocks versioning
                {
            ...
            var temp = new TempContent<Content>(dto.NodeId,0, 0, contentType, c);
    ...
    private IContent MapDtoToContent(ElementDto dto)
    {
        ...
        try
        {
            ...
            content.Properties = properties[0];
    ...
    protected Sql<ISqlContext> GetBaseQuery(QueryType queryType, bool current)
    {
        ...
        sql
            ...
            // TODO create elementVersion and update all referenced documentVersionDtos
    ...
    protected override IEnumerable<string> GetDeleteClauses()
    {
        var list = new List<string>
        {
            ...
            "DELETE FROM " + Constants.DatabaseSchema.Tables.PropertyDataElement + " WHERE elementId = @id",
    ...
    public override IEnumerable<IContent> GetAllVersions(int nodeId)
    {
        ...
        return MapDtosToContent(Database.Fetch<ElementDto>(sql), true);
    }
    ...
    public override IEnumerable<IContent> GetAllVersionsSlim(int nodeId, int skip, int take)
    {
        ...
        return MapDtosToContent(Database.Page<ElementDto>(pageIndex + 1, take, sql).Items, true,
    ...
    protected override void PersistNewItem(IContent entity)
    {
        ...
        // create the dto
        ElementDto dto = ContentBaseFactory.BuildElementDto(entity, NodeObjectTypeId);
    ...
    private class ContentByGuidReadRepository : EntityRepositoryBase<Guid, IContent>
    {
        ...
        protected override IEnumerable<IContent> PerformGetAll(params Guid[]? ids)
        {
            ...
            return _outerRepo.MapDtosToContent(Database.Fetch<ElementDto>(sql));
        }
    ...
    {
        // local elements do not need a name as they are not routed or shown in trees
        if (content.ParentId == Constants.System.LocalElementParent)
        {
            return;
        }
        ...
        {
            // content is invariant, and invariant elements must have an explicit invariant name
