    ...
    internal class InMemoryModelFactory : IAutoPublishedModelFactory, IRegisteredObject, IDisposable
    {
        ...
        internal Infos EnsureModels()
        {
            ...
            try
            {
                ...
                using (!_profilingLogger.IsEnabled(Cms.Core.Logging.LogLevel.Debug) ? null : _profilingLogger.DebugDuration<InMemoryModelFactory>("Get models.", "Got models."))
                {
                    try
                    {
        ...
        public string PureLiveDirectoryAbsolute() => _hostingEnvironment.MapPathContentRoot(Cms.Core.Constants.SystemDirectories.TempData + "/InMemoryAuto");
